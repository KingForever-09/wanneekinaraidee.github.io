...
const { appendUserRow } = require('../google/sheetsSync');
...
function publicUser(row) {
  if (!row) return null;
  return { id: row.id, username: row.username, email: row.email, isAdmin: isAdminEmail(row.email) };
}
...
router.post('/signup', (req, res) => {
  ...
  req.session.regenerate((err) => {
    ...
    // One-way mirror only — the database above is what login actually
    // checks. The sheet exists purely so signups are visible at a glance.
    appendUserRow({
