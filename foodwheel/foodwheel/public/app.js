...
function renderTagBadges(tags) {
  ...
  return `<div class="tag-badges">${tags.map(t => `<span class="tag-badge">${t.emoji} ${t.label}</span>`).join('')}</div>`;
}
...
/* ========================================================
   SEARCH — by name, or by nutrition tag (e.g. "low calorie",
   "healthy food"). Tags are computed server-side from the actual
   kcal/protein/carbs/fat numbers, so they can't drift out of sync.
   ======================================================== */
let searchTags = [];        // [{ slug, label, emoji }]
let activeTagFilter = null; // slug | null
let searchDebounceTimer = null;
const searchResultsCache = {}; // foodId -> food (with tags), so a search-result click can open the result modal
...
async function loadSearchTags() {
  ...
  } catch (_) { /* chips are a nice-to-have — search box still works without them */ }
}

function renderSearchTagChips() {
  const wrap = document.getElementById('searchTagChips');
  wrap.innerHTML = searchTags.map(t => `
    <button type="button" class="tag-chip ${activeTagFilter === t.slug ? 'active' : ''}" data-slug="${t.slug}">
      ${t.emoji} ${t.label}
    </button>
  `).join('');
  wrap.querySelectorAll('.tag-chip').forEach(chip => {
    chip.onclick = () => {
      activeTagFilter = activeTagFilter === chip.dataset.slug ? null : chip.dataset.slug;
      renderSearchTagChips();
      runSearch();
    };
  });
...
function handleSearchInput() {
  ...
  document.getElementById('searchClearBtn').style.display = document.getElementById('searchInput').value ? '' : 'none';
}
...
async function runSearch() {
  ...
  if (!q && !activeTagFilter) {
    resultsEl.innerHTML = '';
    return;
  }

  resultsEl.innerHTML = '<p class="loading-text">กำลังค้นหา...</p>';
  try {
    const params = new URLSearchParams();
    if (q) params.set('q', q);
    if (activeTagFilter) params.set('tag', activeTagFilter);
    const { foods } = await api('/foods/search?' + params.toString());

    if (foods.length === 0) {
      resultsEl.innerHTML = '<p class="loading-text">ไม่พบเมนูที่ตรงกับคำค้นหา ลองคำอื่นดูไหม?</p>';
      return;
    }

    resultsEl.innerHTML = `<p class="search-count">พบ ${foods.length} เมนู</p><div class="menu-grid" id="searchResultsGrid"></div>`;
    ...
    foods.forEach(item => {
      ...
      card.onclick = () => showResultModal(item, item.category_key);
      card.innerHTML = `
        <div class="num">${item.category_emoji || ''} ${item.category_label || item.category_key}</div>
        <h4>${item.name}</h4>
        <img src="${pickImg(item.img)}" alt="${item.name}" loading="lazy" onerror="this.src='${FALLBACK_IMAGE}'">
        <div class="kcal-badge">${item.kcal} kcal</div>
        ${renderTagBadges(item.tags)}
    ...
    resultsEl.innerHTML = `<p class="loading-text">ค้นหาไม่สำเร็จ: ${err.message}</p>`;
  }
}

document.getElementById('searchInput').addEventListener('input', handleSearchInput);
document.getElementById('searchInput').addEventListener('keydown', (e) => {
  if (e.key === 'Enter') { e.preventDefault(); clearTimeout(searchDebounceTimer); runSearch(); }
});
